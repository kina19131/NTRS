...
**⚠️ FURTHER CORRECTION 2026-08-02, RETRACTED SAME DAY — the "containment doesn't hold" claim below was itself wrong, caused by using the wrong baseline NLL.** A review flagged the [0.133, 0.233] classification as inconsistent with the stated 1%-threshold criterion, using a threshold of 1%×3.05859375=0.030586. That 3.05859375 is `nll_baseline` from `pretrained_density_relslack.json` — the value used to *calibrate σ½_pre itself*, computed by `recompute_h1_sigma_relslack.py` on its own small WikiText-2 sample. It is **not** the baseline the fine-tuning table's own `delta_nll_wiki` values are computed against — that baseline is `nll_wiki_base`, stored directly in the table above (3.36375, identical for every row, confirmed identical to the SST-2 table's own `nll_wiki_base`). Two different scripts evaluate WikiText-2 NLL on the same frozen checkpoint slightly differently (different sample counts/seeds), producing two legitimately different numbers — the T* criterion must use the one the ΔNLL column was actually computed against (3.36375), not the unrelated σ½-calibration figure.

**Redone with the correct baseline (1%×3.36375=0.033638): ΔNLL at lr=5e-5 (0.032188) is below threshold — genuinely safe.** This restores the original classification exactly: **Llama MNLI T* = [0.1331, 0.2329]** (block-scope, matching the previously-documented [0.133, 0.233]), and it remains **fully contained** inside Mistral's [0.0715, 0.3243]. Also re-verified under the corrected LoRA-scope σ½_pre: Llama=[0.0677, 0.1184], still fully contained inside Mistral's lora-scope bracket [0.0372, 0.1686]. **The original "fully contained" claim was correct all along; this document's own 2026-08-02 "correction" above was the error, not the thing it was correcting.** Left in place (not deleted) so the two-baseline confusion is documented for anyone editing this file in the future — if a future edit needs "1% of baseline NLL" for a Llama/Mistral fine-tuning comparison, use each table's own `nll_wiki_base` column, never the separate σ½-calibration file's `nll_baseline`.
