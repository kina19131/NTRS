...
**A process note worth keeping on record**: on 2026-08-02, a review flagged this containment claim as wrong, using a threshold computed from `nll_baseline` = 3.05859375 — a figure from `pretrained_density_relslack.json`, the file that calibrates σ½_pre itself. That number is *not* what the fine-tuning table's own `ΔNLL` values are computed against; the correct baseline is that table's own `nll_wiki_base` = 3.36375, identical for every row. Two different scripts evaluate WikiText-2 NLL on the same frozen checkpoint slightly differently (different sample counts/seeds) and produce two legitimately different but easily-confused numbers. Redone with the correct baseline, the original "fully contained" classification is exactly right — the "correction" was itself the error, and was retracted the same day. **Standing rule going forward: for any Llama/Mistral fine-tuning NLL-threshold comparison, always use that table's own `nll_wiki_base` column, never the separate σ½-calibration file's `nll_baseline`.** Full trace in RAW_DATA.md's H1 MNLI section.
