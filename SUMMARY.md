...
**⚠️ FURTHER CORRECTED 2026-08-02, RETRACTED SAME DAY — the claim below (that containment doesn't hold) was itself wrong.** It used 1%×3.05859375=0.030586 as the threshold, but 3.05859375 is `nll_baseline` from the *σ½-calibration* file (`pretrained_density_relslack.json`, a different script's own WikiText-2 measurement) — not the baseline the fine-tuning table's `delta_nll_wiki` values are actually computed against, which is that table's own `nll_wiki_base` (3.36375, same value for every Llama MNLI/SST-2 row). Using the correct baseline: 1%×3.36375=0.033638, and ΔNLL=0.032188 at lr=5e-5 is genuinely below it — safe, exactly as originally classified. Recomputed: Llama MNLI T*=[0.1331,0.2329] (matches the original [0.133,0.233]), **still fully contained** inside Mistral's [0.0715,0.3243] — in both σ½ scope conventions. **The original "fully contained" claim was correct; retracting this entry's correction, not the original text.** Left visible (not deleted) as a reference: for any future Llama/Mistral fine-tuning NLL-threshold comparison, use each table's own `nll_wiki_base` column, never the separate σ½-calibration file's `nll_baseline` — see `RAW_DATA.md`'s H1 MNLI table for the full trace.
...
**Severity sweep + NaN diagnostic, Llama (2026-07-30) — the mechanism question above is now resolved, and a new complication surfaced.** Extended `h2_probe_recovery.py` with a direct check of whether the frozen backbone's hidden states are NaN/Inf, and reran with 2 intermediate LR points (6e-4, 8e-4) and a doubled probe budget (400 steps). **Result: `phase2_hidden_nan`/`probe_recovery_hidden_nan` are `False` in all 12 conditions tested, including every plateaued one** — the training-instability/NaN hypothesis proposed as an open alternative is now ruled out. The lr=1e-3 collapse (whose exact value, 0.478, already independently reproduces a number on record from a different script months earlier) is confirmed to be a real, finite-valued representational phenomenon, not a numerical artifact — though this doesn't by itself prove the strongest "genuine information destruction" reading, since a finite-but-degenerate (e.g. very-low-variance) representation is equally consistent with these results.

The new complication: **AGNews's collapse is genuinely non-monotonic in LR**, not simply harder-to-recover with severity. 5e-4 shows real partial recovery (peaks ~0.90); 6e-4 collapses to a flat, non-learning chance-level oscillation; **8e-4 recovers again** — a real, sustained climb to a stable ~0.80 plateau verified across the full 400-step curve, not a fluke endpoint — then 1e-3 collapses again. MNLI, by contrast, shows a clean, sharp on/off transition (fully recovers at 5e-4/6e-4, fully collapses at 8e-4/1e-3). This means R_A (a magnitude) may predict *whether the old classifier breaks* reasonably well without predicting *whether a fresh probe can fix it* — recovery difficulty appears to depend on which direction training took, not just how far, echoing the same "direction, not just magnitude, matters" pattern already found in H1's direct-R sweep and the SST-2 anisotropy work. Single seed — AGNews's non-monotonicity specifically should be treated as unreplicated until checked with a different data/probe seed.

**⚠️ MAJOR CAVEAT, found 2026-07-30, same day — "confirmed real, finite-valued phenomenon" above is premature.** Tallying every accuracy value across all 27 locally-held result files: 0.5220 and 0.4780 (which sum to exactly 1.000, and exactly equal 261/500 and 239/500 on the fixed SST-2 validation split used everywhere, `seed=42`) are the two most common values by an order of magnitude (372 and 295 occurrences), and **11 of 27 conditions' final recorded value is exactly one of these two** — every Llama `no_recovery` condition in both runs, plus GPT-2's lr=2e-3/5e-3. This is the signature of a classifier converging to a single constant predicted class, mechanically producing one of these two accuracies regardless of the actual hidden states — consistent with either genuine collapse *correctly detected* (no signal → converge to majority class is the expected, correct behavior) or the probe's own optimization getting stuck in a degenerate local minimum despite real signal being present. The same hyperparameters demonstrably do learn well past these values in every condition that shows real recovery, which favors the former over a *blanket* optimizer failure — but doesn't rule it out specifically for the stuck conditions. **This couldn't be checked from data already collected** (only aggregate accuracy was recorded, never the predicted-class distribution) — fixed by adding `_prediction_class_counts` to `h2_probe_recovery.py` (2026-07-30), not yet re-run. Until it is: read "NaN ruled out" as solid, but "genuine finite-valued collapse confirmed" as **not yet distinguished from a probe-training artifact** — and treat the AGNews non-monotonicity finding as needing this same check in addition to seed replication, since its key collapse point (agnews/6e-4 = 0.522) is exactly the suspicious constant. Full data: `RAW_DATA.md § "H2: Fresh-Probe Recovery — Llama-3.2-3B Severity Sweep + NaN Diagnostic"`.

**Generality test — does readout-mismatch hold for a Phase-1 task that wasn't the SST-2 anomaly? (2026-07-30/31)** Every fresh-probe experiment so far used SST-2 as Phase 1, because SST-2 was the identified anomaly. This left the bigger question untested: is readout-mismatch an SST-2 peculiarity, or does it apply to the Phase-1 tasks (MNLI, AGNews, CoLA) whose H2 transfer results already looked clean — which would mean reframing what T* measures throughout the *entire* H2 framework, not just one footnote.

GPT-2 couldn't be used for this (no MNLI Phase-1 checkpoint existed, and 3 separate training attempts — varying LR 2.5× and LoRA rank 4× — all failed to learn MNLI at all, loss flat at exactly `ln(3)`, the random-3-class-guess value, the entire time; a tokenizer-truncation hypothesis was checked directly and ruled out; most likely a genuine GPT-2 capacity limit for 3-way NLI, not a bug). Pivoted to Llama's existing, already-validated MNLI checkpoint (88.4% accuracy) instead — no retraining needed.

Tested `mnli→{sst2, agnews, cola}` at lr=5e-4 (old-classifier accuracy collapses to 0.298/0.384/0.318 in all three — comparably catastrophic to SST-2's worst cases), escalating the probe budget across 4 rounds (200→400→800→2000 steps) because each round's curves kept climbing rather than plateauing — this project's standing stopping rule is a genuine plateau, not a step-count target. **The class-count diagnostic caught a real artifact on its first live use**: at 400 steps, `cola`'s fresh probe had collapsed to predicting one class 99.2% of the time (`{1: 496, 2: 4}`) — re-run with a lower `probe_lr` (3e-4) resolved the collapse, confirming it was a probe-optimization artifact, not evidence about the representation.

**Result, after 4 rounds — revised 2026-08-02 after closer scrutiny of the per-condition data quality**: **zero of three conditions crossed the recovery threshold, at any budget tested** — a real, qualitative difference from SST-2-as-Phase-1, where all three GPT-2 conditions crossed cleanly within 55–200 steps. The three targets are not equally solid evidence even for the weaker "same qualitative signature" claim: `sst2` is the strong case (peaked at 0.798 vs. 0.7956, a 0.0024 near-miss, decelerating but plausibly still closing); `agnews` is genuine but weaker (0.598, decelerating faster, 0.198 short — though checked and ruled out that this is majority-class bias: the classifier's most-predicted class alone can't explain more than ~33% accuracy on MNLI's balanced 3-way validation set, so the 59.8% achieved requires real signal); `cola` turned out to have **no valid trend data at all** — its `v1` measurement (200 steps, same default `probe_lr` later confirmed to cause the `v2` collapse) shows the identical shape of degeneracy (`{'2': 192, '0': 306, '1': 2}`, class 1 barely predicted), just less extreme, meaning only the single `v3` measurement (post-fix) is trustworthy — there is no clean before/after comparison for cola, contrary to the original "weak but real improvement" framing. **Net**: MNLI shows the same qualitative signature as SST-2 (collapse + non-degenerate recovery direction, not a chance-level plateau) but has not reached the SST-2-established recovery bar for any condition — a partial, unresolved generalization test, not a confirmed extension. Separately, since GPT-2 never produced a usable MNLI checkpoint, this generality test has no GPT-2/Llama cross-architecture check at all, unlike the original SST-2 investigation.

**Net effect**: readout-mismatch is not an SST-2-only artifact — it generalizes to MNLI, at least partially, though slower and less complete than SST-2's own recovery. This means H2's classifier-based "forgetting" signal likely carries a real readout-mismatch component *broadly*, not just in the one place it was first noticed — a materially bigger reframing than the original V8 investigation anticipated. Single run/seed throughout, no replication; the exact asymptotes for sst2/agnews at much larger budgets, and cola's asymptote beyond 800 steps, remain genuinely unknown. Full data: `RAW_DATA.md § "H2: Fresh-Probe Recovery Generality Test — MNLI as Phase-1 Task"`.
...
**⚠️ Escalation, 2026-07-30/31, revised 2026-08-02 — this is no longer just about SST-2, though the evidence is weaker than first written up.** Tested whether readout-mismatch also applies to MNLI (a Phase-1 task whose H2 transfer results already looked clean, i.e. not part of the original V8 anomaly). Result, after closer scrutiny: none of the three tested MNLI Phase-2 targets (sst2, agnews, cola) actually crossed the recovery threshold — sst2 came within 0.0024 of it, agnews recovered substantially but incompletely (with majority-class bias checked and ruled out via direct arithmetic), and cola's apparent "trend" turned out to be a single clean data point with no valid before/after comparison. So this is **not confirmed as a generalization of readout-mismatch** — but it does establish that MNLI's forgetting is *not* the clean, "genuinely gone" case that would have reassured the rest of H2 either: every tested condition showed real, non-degenerate recovery direction, just short of the established bar. V8 is no longer safely contained to explaining one anomalous task — the honest state is "not ruled out as a broader property, not yet confirmed as one either." See "Generality test" subsection above.
