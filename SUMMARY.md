...
**Result, after 4 rounds**: `sst2` came within 0.002 of the recovery threshold (peaked at 0.798 vs. 0.7956) before the trend visibly slowed — an apparent near-miss, not a clean recovery or a clean failure. `agnews` recovered substantially (0.384→~0.63) but with clearly slowing increments, short of the threshold. `cola` recovered only weakly even after the collapse fix. All three showed genuine, non-collapsed, verified-via-prediction-distribution learning — **none plateaued at a degenerate or chance-level state**, which would have been the reassuring "MNLI forgetting is genuine, SST-2 is special" outcome. Stopped escalating at this point (diminishing per-round increments on both remaining curves) rather than chase a fully clean crossing at increasing cost.
...
**⚠️ Escalation, 2026-07-30/31 — this is no longer just about SST-2.** Tested whether readout-mismatch also applies to MNLI (a Phase-1 task whose H2 transfer results already looked clean, i.e. not part of the original V8 anomaly) — found that it does, at least partially: all three tested MNLI Phase-2 targets (sst2, agnews, cola) showed real, non-collapsed recovery from catastrophic old-classifier failure, with sst2 coming within 0.002 of the full recovery threshold. This means V8 was never just about explaining one anomalous task's variability — it points toward H2's classifier-based forgetting signal carrying a real readout-mismatch component *across Phase-1 tasks generally*, which is a materially larger reframing than "SST-2 is an instructive edge case." See "Generality test" subsection above.
